...
export default function NovoUsuario() {
  ...
  const handleCreated = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);

    if (!nome || !email || !senha || !sexo || !contato) {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}api/usuario`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setError(errorData.mensagem || "Erro ao criar usuário");
      }
    } catch (err) {
